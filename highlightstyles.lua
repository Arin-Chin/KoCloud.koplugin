...
--   KoCloud.koplugin now mounts this file itself (highlightstyles.lua)
--   at startup, so the standalone patches/ copy is no longer needed. If you
--   keep BOTH, the built-in duplicate guards make it harmless.
...
-- ─────────────────────────────────────────────────────────────────────────
-- 🇨🇳 Localization -- show Chinese labels when KOReader's UI language is
--     Chinese (zh_*), otherwise fall back to gettext (English defaults).
--     (KOReader applies the language setting at startup, so checking once
--     here is enough: changing the UI language restarts KOReader.)
-- ─────────────────────────────────────────────────────────────────────────
local ZH = {
    ["Underline"] = "下划线",
    ["Wavy"] = "波浪线",
    ...
    ["thickness"] = "粗细",
    ["Apply"] = "应用",
    ["Highlight Style Thickness"] = "高亮样式粗细",
}
local lang_locale = G_reader_settings:readSetting("language") or ""
local is_zh = lang_locale:sub(1, 2) == "zh"
local function L(msgid)
    if is_zh then
...
local STYLE_OPTIONS = {
    { key = "underscore",        label = L("Underline"),         thickness_key = "underline" },
    { key = "wavy",              label = L("Wavy"),              thickness_key = "wavy" },
    { key = "squiggly",          label = L("Squiggly"),          thickness_key = "squiggly" },
    { key = "dash",              label = L("Dash"),              thickness_key = "dash" },
    { key = "dot",               label = L("Dot"),               thickness_key = "dot" },
    { key = "double_underline",  label = L("Double underline"),  thickness_key = "double_underline" },
    { key = "zigzag",            label = L("Zig-zag"),           thickness_key = "zigzag" },
    { key = "circle",            label = L("Circle"),            thickness_key = "circle" },
    { key = "rectangle",         label = L("Rectangle"),         thickness_key = "rectangle" },
}
...
ReaderView.drawHighlightRect = function(self, bb, _x, _y, rect, drawer, color, draw_note_mark)
    ...
    if drawer == "underscore" and LINE_THICKNESS.underline then
        ...
        local ok, err = pcall(orig_drawHighlightRect, self, bb, _x, _y, rect, drawer, color, draw_note_mark)
...
local ok_menu, menu_err = pcall(function()
    ...
    function ThicknessMenu:addToMainMenu(menu_items)
        ...
        for _, entry in ipairs(STYLE_OPTIONS) do
            local tk = entry.thickness_key or entry.key
            table.insert(sub_item_table, {
                text_func = function()
                    return entry.label .. ": " .. tostring(LINE_THICKNESS[tk]) .. "px"
                end,
                keep_menu_open = true,
                callback = function()
                    showThicknessPickerFromMenu(tk, entry.label)
                end,
            })
        end
        menu_items.fancy_highlight_thickness = {
            text = L("Highlight Style Thickness"),
