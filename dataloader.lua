...
function DataLoader:getHistoryItems()
    ...
    if not ok or type(history) ~= "table" then
        logger.warn("KoCloud: Failed to load history.lua")
        return {}
    end
    return history
end
...
function DataLoader:getBookTimeline(book_ref)
    ...
    if not load_ok then
        logger.warn("KoCloud: Failed to load lua-ljsqlite3:", SQ3)
        return {
            book_id = book.id,
            book_ref = book.id,
            title = book.title,
    ...
    if not open_ok then
        logger.warn("KoCloud: Failed to open statistics db:", conn)
        return {
            book_id = book.id,
            book_ref = book.id,
            title = book.title,
