...
function Api.handleRequest(server, reqinfo, path, full_uri)
    if path == "/api/server/stop" then
        ...
        local enc_ok, json_str = pcall(JSON.encode, {
            ok = true,
            stopping = true,
            message = "KoCloud server is stopping",
        })
        if not enc_ok then
        ...
        UIManager:scheduleIn(0.05, function()
            ...
            if not ok_stop then
                logger.err("KoCloud: failed to stop server from API:", tostring(stop_err))
            end
        end)
        return ev
    end
    ...
    if path == "/api/cloud/annotations/sync" then
        ...
        local started = Sync.startJob("annotations", true)
        local payload = {
            ok = started,
            running = started,
        }
        return server:sendResponse(reqinfo, 200, CTYPE.JSON, JSON.encode(payload))
    end
    ...
    if path == "/api/cloud/progress/sync" then
        ...
        local started = Sync.startJob("progress", true)
    ...
    if path == "/api/cloud/covers/backup" or path == "/api/cloud/covers/restore" then
        ...
        local kind = path:match("/api/cloud/covers/(%w+)") == "restore" and "covers-restore" or "covers-backup"
        local started = Sync.startCoverJob(kind)
        return server:sendResponse(reqinfo, 200, CTYPE.JSON, JSON.encode({ ok = started, running = started }))
    end
    ...
    if not enc_ok then
        logger.err("KoCloud JSON encode error:", json_str)
        return server:sendResponse(reqinfo, 500, CTYPE.JSON,
...
function Api.sendBookCover(server, reqinfo, book_ref)
    ...
    if not ok then
        logger.err("KoCloud cover API error:", cover_info)
...
function Api.route(path, full_uri, reqinfo)
    ...
    if path == "/api/cloud/status" then
        return Api.getCloudStatus()
    end

    if path == "/api/cloud/activity" then
        return Api.getCloudActivity()
    end

    return { error = "Unknown API endpoint: " .. path }
end

function Api.getCloudStatus()
    if not Sync.available() then
        return { available = false, configured = false, reason = "cloud storage support unavailable" }
    end
    local s = Sync.getSettings()
    local configured = type(s.sync_server) == "table"
    local ps = Sync.getProgressSettings()
    local p_configured = type(ps.server) == "table"
    return {
        available = true,
        configured = configured and s.is_enabled ~= false,
        provider = configured and s.sync_server.type or nil,
        name = configured and s.sync_server.name or nil,
        path = configured and Sync.readableServerPath(s.sync_server) or nil,
        auto = {
            open = s.sync_on_open == true,
            close = s.sync_on_close == true,
            resume = s.sync_on_resume == true,
        },
        progress = {
            configured = p_configured,
            provider = p_configured and ps.server.type or nil,
            name = p_configured and ps.server.name or nil,
            path = p_configured and Sync.readableServerPath(ps.server) or nil,
            conflict = ps.conflict or "later",
            auto = {
                open = ps.auto.open == true,
                close = ps.auto.close == true,
                resume = ps.auto.resume == true,
            },
        },
        busy = Sync.isBusy(),
    }
